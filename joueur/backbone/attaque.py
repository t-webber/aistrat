...
def move_everyone(player, token, case, allies_voisins, knights):
    ...
    for i in allies_voisins:
        for j in range(0, allies_voisins[i]):
            api.move(
                api.KNIGHT, Y + allies_voisins[i][0], X + allies_voisins[i][1], Y, X, player, token)
            knights.remove(
                (Y + allies_voisins[i][0], X + allies_voisins[i][1]))
...
def attaque(player, case_attaquee, knights, token):
    ...
    defenseurs_voisins = compte_soldats_ennemis_cases_adjacentes(
        player, case_attaquee)
