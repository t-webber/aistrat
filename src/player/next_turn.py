...
def nexturn_fl(player, token):
    ...
    if len(fl.front_line[player]) == 0:
        fl.front_line[player] = fl.create_front(player)
    
    fl.front_line[player] = fl.update_front(player, knights)

    fl.is_full[player] = fl.is_full[player] or fl.is_full_update[player]

    if not fl.is_full[player]:
        fl.move_to_line(knights, player, token)
        fl.equilibrate(knights, player, token)
    
    if not fl.broken_line(player):
